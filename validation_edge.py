...
def create_gene2go_matrix(biomart,exp_profile,graph_dict,threads) :
    ...
    gene2go_matrix = sparse.dok_matrix((len(gene_list), len(goindex)),dtype=np.int8) # type igonore
...
def validate_edge(edges,gene2go_matrix,index2go,corr_threshold = 0.5) :
    '''
...
def main() :
    ...
    result = pool.map(partial(validate_edge,gene2go_matrix=gene2go_csr,index2go=index2go,corr_threshold=corr_threshold),edge_with_corr_list[:100])
    pool.close()
    pool.join()
    #memory record
